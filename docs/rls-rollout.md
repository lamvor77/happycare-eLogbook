...
> **읽기 전 주의 (2026-09-01 추가)**
>
> 이 문서는 2026-08-03 RLS 적용 당시의 *절차 기록*이다. 그 뒤 간병인 인증이
> Supabase Auth에서 자체 세션 쿠키로 바뀌면서, 여기 나오는 간병인용 RLS
> 정책들은 실제 트래픽에서 더 이상 평가되지 않는다(`auth.uid()`가 없어
> 항상 거짓 — fail-closed). **"caregiver RLS가 이중 방어한다"는 이 문서의
> 서술은 현재 사실이 아니다.** 간병인 경로의 방어선은 서버 라우트
> (`lib/caregiver-auth.ts`)뿐이고, 관리자 경로만 RLS(`is_admin()`)를 계속
> 사용한다.
>
> 현재 보안 모델은 `docs/data-model.md` 4.3~4.4절을 참고할 것. 아래 본문은
> 당시 기록으로 보존한다(사후에 고쳐 쓰지 않는다).
