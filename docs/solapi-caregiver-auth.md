...
- 코드 형식: 4자리 숫자(`0000`~`9999`, 앞자리 0 유지), `crypto.randomInt`로
  생성(2026-08-26 6자리→4자리 변경 — 코드 원문을 저장하지 않고 해시만
  저장하므로 DB 스키마 변경은 없음).
- 만료: 발송 후 **5분**.
- 재발송 제한: 동일 번호로 **60초** 이내 재발송 불가.
- 일일 발송 한도: 동일 번호 **하루 10회**.
- 실패 제한: 코드 불일치가 **3회** 누적되면 해당 OTP 건은 더 이상
  검증할 수 없다(새로 발송받아야 함, 4자리 전환에 맞춰 5회->3회로
  강화, 2026-08-26).
