 ...
 *
 * 근거(운영 기준):
 *   - lib/legacy-sync.ts는 Apps Script 요청에 30초(REQUEST_TIMEOUT_MS)
 *     타임아웃을 직접 걸고 있다. 즉 외부 HTTP 왕복은 길어도 30초에서
 *     끊긴다(2026-08-27에 10초에서 상향 — 정상 처리 건을 timeout으로
 *     오판하는 문제가 있었다).
 ...
 * 대비 충분히 보수적인 복구 시점)의 의미는 그대로 유지된다.
 *
 * REQUEST_TIMEOUT_MS를 10초에서 30초로 올린 2026-08-27에 이 값을 함께
 * 재검토했고, 그대로 5분을 유지하기로 했다 — 30초는 5분보다 한 자릿수
 * 짧아서 "정상 처리 중인 pending"이 이 기준을 넘길 여지가 여전히 넉넉하다.
 * 앞으로 타임아웃을 5분에 근접하게 올리는 변경이 생긴다면 그때는 이 값도
 * 반드시 함께 올려야 한다.
 */
